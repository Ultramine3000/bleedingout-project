...
func _physics_process(delta: float) -> void:
	...
	var input_dir = Input.get_vector("p1_move_left", "p1_move_right", "p1_move_forward", "p1_move_backward")
	...
	if Input.is_action_pressed("p1_aim"):
		is_aiming = true
	else:
		is_aiming = false
...
func _handle_head_tilt(delta: float):
	...
	if Input.is_action_pressed("p1_lean_left"):
		target_tilt_angle = MAX_TILT_ANGLE
	elif Input.is_action_pressed("p1_lean_right"):
		target_tilt_angle = -MAX_TILT_ANGLE
