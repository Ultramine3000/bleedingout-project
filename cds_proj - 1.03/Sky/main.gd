...
func _process(delta: float) -> void:
	if Engine.is_editor_hint(): # We don't want a time lapse in the editor
		return
...
func _update_sun() -> void:
	if is_instance_valid(sun):
		...
		var earth_orbit_progress: float = \
		  (float(day_of_year) + 193.0 + day_progress) / float(DAYS_IN_YEAR)
		...
		var sun_direction: Vector3 = sun.to_global(Vector3(0.0, 0.0, 1.0)).normalized()
...
func _update_moon() -> void:
	...
	if is_instance_valid(moon):
		...
		var axial_tilt: float = moon_orbital_inclination
		...
		# Disabling light under the horizon
		var moon_direction: Vector3 = moon.to_global(Vector3(0.0, 0.0, 1.0)).normalized()
...
func _update_shader() -> void:
	if not is_instance_valid(environment):
		return
	environment.environment.sky.sky_material.set_shader_parameter("overwritten_time", \
